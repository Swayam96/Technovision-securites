...
export default function SupplierPaymentForm() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`/api/purchase/payments/${id}`, { withCredentials: true });
      const data = res.data;
      if (data.date) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id && id !== 'new') {
        await axios.put(`/api/purchase/payments/${id}`, formData, { withCredentials: true });
      } else {
        await axios.post(`/api/purchase/payments`, formData, { withCredentials: true });
      }
      navigate('/modules/purchase-procurement/payments');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 text-gray-800 font-weight-bold">{id === 'new' ? 'Create' : 'Edit'} Payment</h4>
      </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="row mb-3">
              <div className="col-md-6">
                ...
                <input type="text" className="form-control" name="payment_no" value={formData.payment_no || ''} onChange={handleChange} required />
              </div>
              <div className="col-md-6">
            ...
            <div className="mt-4">
              ...
              <button type="button" onClick={() => navigate('/modules/purchase-procurement/payments')} className="btn btn-light px-4 border">Cancel</button>
            </div>
          </form>
        </div>
      </div>
