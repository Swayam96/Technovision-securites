...
export default function PurchaseReturnForm() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`/api/purchase/returns/${id}`, { withCredentials: true });
      const data = res.data;
      if (data.date) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id && id !== 'new') {
        await axios.put(`/api/purchase/returns/${id}`, formData, { withCredentials: true });
      } else {
        await axios.post(`/api/purchase/returns`, formData, { withCredentials: true });
      }
      toast.success('Saved successfully!');
      navigate('/modules/purchase-procurement/returns');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 text-gray-800 font-weight-bold">{id === 'new' ? 'Create' : 'Edit'} Return</h4>
      </div>
        ...
        <div className="card-body">
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-4">
              ...
              <button type="button" onClick={() => navigate('/modules/purchase-procurement/returns')} className="btn btn-light px-4 border">Cancel</button>
            </div>
          </form>
        </div>
      </div>
