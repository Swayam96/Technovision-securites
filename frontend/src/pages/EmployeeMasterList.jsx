...
export default function EmployeeMasterList() {
  ...
  const fetchEmployees = async (searchQuery = '') => {
    try {
      const res = await axios.get(`/api/employees?q=${searchQuery}`, { withCredentials: true });
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div className="content-header" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          <div className="text-muted small mb-1" style={{ fontSize: '0.85rem' }}>Organisation & Administration &gt; Employee Master</div>
        ...
        <div>
          ...
          >
            <i className="fas fa-plus mr-1" style={{ marginRight: '5px' }}></i> Add Employee
          </button>
        </div>
      </div>
        ...
        <div className="card-body table-responsive p-0">
          <table className="table table-hover mb-0" style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ background: '#f8fafc', borderBottom: '2px solid #eee' }}>
              <tr>
                <th style={{ padding: '12px 20px', textAlign: 'left' }}>Employee Name</th>
                ...
                employees.map(e => (
                  <tr key={e.id} style={{ borderBottom: '1px solid #eee' }}>
                    <td style={{ padding: '15px 20px' }}>
                      <strong className="text-dark">{e.employee_name}</strong>
                      <div style={{ fontSize: '0.8rem', color: '#64748b' }}>{e.username}</div>
                    </td>
                    <td style={{ padding: '15px 20px' }}>{e.company_name || '—'}</td>
                    <td style={{ padding: '15px 20px' }}>{e.department_name || '—'}</td>
                    <td style={{ padding: '15px 20px' }}>{e.designation_name || '—'}</td>
                    <td style={{ padding: '15px 20px' }}>{e.role_name || '—'}</td>
                    <td style={{ padding: '15px 20px' }}>
                      <span style={{ 
                    ...
                    <td style={{ padding: '15px 20px', textAlign: 'center' }}>
                      <button 
                        onClick={() => navigate(`/modules/organization-administration/employee-master/${e.id}`)}
                ...
                <tr>
                  <td colSpan="7">
                    <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>
                       <i className="fas fa-id-badge fa-3x mb-3 text-light" style={{ fontSize: '3rem', color: '#cbd5e1', marginBottom: '15px', display: 'block' }}></i>
                       <p className="mb-0">{query ? 'No matches.' : 'No employees yet.'}</p>
                    </div>
                  </td>
                </tr>
              )}
