...
export default function ServiceHistoryForm() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`/api/assets/service-history/${id}`, { withCredentials: true });
      const data = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id && id !== 'new') {
        await axios.put(`/api/assets/service-history/${id}`, formData, { withCredentials: true });
      } else {
        await axios.post(`/api/assets/service-history`, formData, { withCredentials: true });
      }
      toast.success('Saved successfully!');
      navigate('/modules/assets-installed-base/service-history');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 text-gray-800 font-weight-bold">{id === 'new' ? 'Create' : 'Edit'} Service History</h4>
      </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="row mb-3">
              <div className="col-md-6">
                ...
                <input type="text" className="form-control" name="ticket_no" value={formData.ticket_no || ''} onChange={handleChange} required />
              </div>
              {apiPath !== 'locations' && (
                <div className="col-md-6">
            ...
            <div className="mt-4">
              ...
              <button type="button" onClick={() => navigate('/modules/assets-installed-base/service-history')} className="btn btn-light px-4 border">Cancel</button>
            </div>
          </form>
        </div>
      </div>
