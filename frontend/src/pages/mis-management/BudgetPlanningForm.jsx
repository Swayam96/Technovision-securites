...
export default function BudgetPlanningForm() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`/api/mis/budget-planning/${id}`, { withCredentials: true });
      const data = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id && id !== 'new') {
        await axios.put(`/api/mis/budget-planning/${id}`, formData, { withCredentials: true });
      } else {
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 text-gray-800 font-weight-bold">{id === 'new' ? 'Create' : 'Edit'} Budget & Planning</h4>
      </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="row mb-3">
              <div className="col-md-6">
                ...
                <input type="text" className="form-control" name="financial_year" value={formData.financial_year || ''} onChange={handleChange} required />
              </div>
              <div className="col-md-6">
            ...
            <div className="mt-4">
              ...
              <button type="button" onClick={() => navigate('/modules/mis-management/budget-planning')} className="btn btn-light px-4 border">Cancel</button>
            </div>
          </form>
        </div>
      </div>
