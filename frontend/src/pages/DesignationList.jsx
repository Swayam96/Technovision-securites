...
export default function DesignationList() {
  ...
  const fetchDesignations = async (searchQuery = '') => {
    try {
      const res = await axios.get(`/api/designations?q=${searchQuery}`, { withCredentials: true });
      ...
      <div className="content-header" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
        ...
        <div>
          <button 
            ...
            onClick={() => navigate('/modules/organization-administration/designations/new')}
          >
        ...
        <div className="card-body table-responsive p-0">
          <table className="table table-hover mb-0" style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ background: '#f8fafc', borderBottom: '2px solid #eee' }}>
              <tr>
                <th style={{ padding: '12px 20px', textAlign: 'left' }}>Designation Name</th>
              ...
              {loading ? (
                ...
                designations.map(r => (
                  <tr key={r.id} style={{ borderBottom: '1px solid #eee' }}>
                    <td style={{ padding: '15px 20px' }}><strong className="text-dark">{r.designation_name}</strong></td>
                    <td style={{ padding: '15px 20px' }}>{r.designation_code}</td>
                    ...
                    <td style={{ padding: '15px 20px', textAlign: 'center' }}>
                      <button 
                        onClick={() => navigate(`/modules/organization-administration/designations/${r.id}`)}
                ...
                <tr>
                  <td colSpan="6">
                    <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>
                       <i className="fas fa-sitemap fa-3x mb-3 text-light" style={{ fontSize: '3rem', color: '#cbd5e1', marginBottom: '15px', display: 'block' }}></i>
                       <p className="mb-0">{query ? 'No matches.' : 'No designations yet. Add the first designation.'}</p>
                    </div>
                  </td>
                </tr>
              )}
