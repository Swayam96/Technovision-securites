...
export default function DepartmentList() {
  ...
  const fetchDepartments = async (searchQuery = '') => {
    try {
      const res = await axios.get(`/api/departments?q=${searchQuery}`, { withCredentials: true });
      ...
      <div className="content-header" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
        ...
        <div>
          <button 
            ...
            onClick={() => navigate('/modules/organization-administration/departments/new')}
          >
        ...
        <div className="card-body table-responsive p-0">
          <table className="table table-hover mb-0" style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ background: '#f8fafc', borderBottom: '2px solid #eee' }}>
              <tr>
                <th style={{ padding: '12px 20px', textAlign: 'left' }}>Department Name</th>
              ...
              {loading ? (
                ...
                departments.map(r => (
                  <tr key={r.id} style={{ borderBottom: '1px solid #eee' }}>
                    <td style={{ padding: '15px 20px' }}><strong className="text-dark">{r.department_name}</strong></td>
                    <td style={{ padding: '15px 20px' }}>{r.department_code}</td>
                    ...
                    <td style={{ padding: '15px 20px', textAlign: 'center' }}>
                      <button 
                        onClick={() => navigate(`/modules/organization-administration/departments/${r.id}`)}
                ...
                <tr>
                  <td colSpan="5">
                    <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>
                       <i className="fas fa-network-wired fa-3x mb-3 text-light" style={{ fontSize: '3rem', color: '#cbd5e1', marginBottom: '15px', display: 'block' }}></i>
                       <p className="mb-0">{query ? 'No matches.' : 'No departments yet. Add the first department.'}</p>
                    </div>
                  </td>
                </tr>
              )}
