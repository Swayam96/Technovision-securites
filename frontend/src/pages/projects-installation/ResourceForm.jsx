...
export default function ResourceForm() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`/api/projects/resources/${id}`, { withCredentials: true });
      const data = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id && id !== 'new') {
        await axios.put(`/api/projects/resources/${id}`, formData, { withCredentials: true });
      } else {
        await axios.post(`/api/projects/resources`, formData, { withCredentials: true });
      }
      navigate('/modules/projects-installation/team');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 text-gray-800 font-weight-bold">{id === 'new' ? 'Create' : 'Edit'} Resource</h4>
      </div>
        ...
        <div className="card-body">
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-4">
              ...
              <button type="button" onClick={() => navigate('/modules/projects-installation/team')} className="btn btn-light px-4 border">Cancel</button>
            </div>
          </form>
        </div>
      </div>
