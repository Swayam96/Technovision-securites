...
export default function BOQForm() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`/api/projects/boq/${id}`, { withCredentials: true });
      const data = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id && id !== 'new') {
        await axios.put(`/api/projects/boq/${id}`, formData, { withCredentials: true });
      } else {
        await axios.post(`/api/projects/boq`, formData, { withCredentials: true });
      }
      navigate('/modules/projects-installation/boq');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 text-gray-800 font-weight-bold">{id === 'new' ? 'Create' : 'Edit'} BOQ</h4>
      </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="row mb-3">
              <div className="col-md-6">
                ...
                <input type="text" className="form-control" name="boq_code" value={formData.boq_code || ''} onChange={handleChange} required />
              </div>
              <div className="col-md-6">
            ...
            <div className="mt-4">
              ...
              <button type="button" onClick={() => navigate('/modules/projects-installation/boq')} className="btn btn-light px-4 border">Cancel</button>
            </div>
          </form>
        </div>
      </div>
