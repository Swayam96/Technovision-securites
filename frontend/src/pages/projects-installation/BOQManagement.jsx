...
export default function BOQManagement() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get('/api/projects/boq', { withCredentials: true });
      setData(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 text-gray-800 font-weight-bold">B O Q Management</h4>
      ...
      <div className="card shadow mb-4 border-0">
        <div className="card-header py-3 bg-white d-flex flex-row align-items-center justify-content-between">
          <h6 className="m-0 font-weight-bold text-primary">All B O Qs</h6>
        </div>
              ...
              <tbody>
                {data.map(item => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <Link to={`/modules/projects-installation/boq/${item.id}`} className="btn btn-sm btn-link text-primary p-0 me-2"><i className="fas fa-edit"></i> Edit</Link>
                    </td>
                  </tr>
                ))}
                {data.length === 0 && (
