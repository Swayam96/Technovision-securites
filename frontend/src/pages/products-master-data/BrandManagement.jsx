...
export default function BrandManagement() {
    ...
    useEffect(() => {
        axios.get('http://localhost:3000/api/products/brands', { withCredentials: true })
            .then(res => {
                setBrands(res.data);
                setLoading(false);
            })
        ...
        <div className="container-fluid py-4">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <div className="text-muted small">Products & Master Data &gt; Brands</div>
                </div>
                <div>
                        ...
                        <div className="table-responsive">
                            <table className="table table-hover align-middle mb-0">
                                <thead className="table-light text-muted small text-uppercase">
                                    <tr>
                                        <th className="ps-4">Brand Name</th>
                                        <th>Country</th>
                                        <th>No. of Items</th>
                                        <th>Status</th>
                                ...
                                <tbody>
                                    {brands.map((brand) => (
                                        <tr key={brand.id}>
                                            <td className="ps-4 fw-medium text-dark">{brand.brand_name}</td>
                                            <td className="text-muted">{brand.country || '-'}</td>
                                            <td className="text-muted">{brand.no_of_items}</td>
                                            <td>{getStatusBadge(brand.status)}</td>
                                            <td className="text-end pe-4">
                                                <Link to={`/modules/products/brands/${brand.id}`} className="btn btn-sm btn-link text-primary p-0 me-2"><i className="fas fa-edit"></i> Edit</Link>
