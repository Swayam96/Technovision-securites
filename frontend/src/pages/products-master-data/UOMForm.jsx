...
export default function UOMForm() {
  ...
  useEffect(() => {
    if (isEdit) {
      axios.get(`http://localhost:3000/api/products/uoms/${id}`, { withCredentials: true })
        .then(res => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await axios.put(`http://localhost:3000/api/products/uoms/${id}`, formData, { withCredentials: true });
      } else {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      <div className="content-header" style={{ marginBottom: '20px' }}>
        <div className="text-muted small mb-1" style={{ fontSize: '0.85rem' }}>Products & Master Data &gt; UOM</div>
        <h1 className="mb-0 d-flex align-items-center" style={{ margin: 0, fontSize: '1.5rem', fontWeight: 600, display: 'flex', alignItems: 'center' }}>
          ...
          {isEdit ? 'Edit UOM' : 'New UOM'}
        </h1>
      </div>

      <form onSubmit={handleSubmit}>
        <div style={cardStyle}>
          <div style={cardHeaderStyle}>UOM Information</div>
          <div style={cardBodyStyle}>
            <div style={gridStyle}>
              <div>
                <label style={labelStyle}>UOM Name *</label>
                <input type="text" name="uom_name" value={formData.uom_name} onChange={handleChange} required style={inputStyle} placeholder="e.g. Nos, Box, Mtr" />
              </div>
              <div>
                <label style={labelStyle}>UOM Type</label>
                <input type="text" name="uom_type" value={formData.uom_type} onChange={handleChange} style={inputStyle} placeholder="e.g. Numbers, Length, Weight" />
              </div>
              <div>
        ...
        <div style={{ textAlign: 'right', marginTop: '20px' }}>
          <button type="button" onClick={() => navigate('/modules/products/uoms')} style={{ background: '#f8fafc', color: '#475569', border: '1px solid #cbd5e1', padding: '10px 20px', borderRadius: '6px', cursor: 'pointer', marginRight: '10px', fontWeight: 500 }}>
            Cancel
          </button>
          <button type="submit" style={{ background: '#3b82f6', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '6px', cursor: 'pointer', fontWeight: 500 }}>
            <i className="fas fa-save mr-1" style={{ marginRight: '5px' }}></i> Save UOM
          </button>
        </div>
      </form>
    </div>
