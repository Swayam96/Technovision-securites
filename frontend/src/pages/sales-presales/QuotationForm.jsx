...
export default function QuotationForm() {
  ...
  useEffect(() => {
    if (isEdit) {
      axios.get(`http://localhost:3000/api/sales/quotations/${id}`, { withCredentials: true })
        .then(res => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await axios.put(`http://localhost:3000/api/sales/quotations/${id}`, formData, { withCredentials: true });
      } else {
        await axios.post('http://localhost:3000/api/sales/quotations', formData, { withCredentials: true });
      }
      navigate('/modules/sales-presales/quotations');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto' }}>
      <div className="content-header" style={{ marginBottom: '20px' }}>
        <div className="text-muted small mb-1" style={{ fontSize: '0.85rem' }}>Sales & Presales &gt; Quotations</div>
      ...
      <form onSubmit={handleSubmit}>
        <div style={cardStyle}>
          <div style={cardHeaderStyle}>Quotation Details</div>
          <div style={cardBodyStyle}>
            <div style={gridStyle}>
              <div>
                <label style={labelStyle}>Quotation No *</label>
                <input type="text" name="qtn_no" value={formData.qtn_no} onChange={handleChange} required style={inputStyle} />
              </div>
              <div>
        ...
        <div style={{ textAlign: 'right', marginTop: '20px' }}>
          <button type="button" onClick={() => navigate('/modules/sales-presales/quotations')} style={{ background: '#f8fafc', color: '#475569', border: '1px solid #cbd5e1', padding: '10px 20px', borderRadius: '6px', cursor: 'pointer', marginRight: '10px', fontWeight: 500 }}>
            Cancel
          </button>
          <button type="submit" style={{ background: '#3b82f6', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '6px', cursor: 'pointer', fontWeight: 500 }}>
            <i className="fas fa-save mr-1" style={{ marginRight: '5px' }}></i> Save Quotation
          </button>
        </div>
      </form>
    </div>
