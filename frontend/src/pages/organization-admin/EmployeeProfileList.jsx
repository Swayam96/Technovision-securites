...
export default function EmployeeProfileList() {
  ...
  const fetchProfiles = async (searchQuery = '') => {
    try {
      const res = await axios.get(`/api/employee-profiles?q=${searchQuery}`, { withCredentials: true });
  ...
  return (
    <div style={{ padding: '20px' }}>
      <div className="content-header" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          <div className="text-muted small mb-1" style={{ fontSize: '0.85rem' }}>Organisation & Administration &gt; Employee Profiles</div>
        ...
        <div>
          <button 
            ...
            onClick={() => navigate('/modules/organization-administration/employee-profile/new')}
          >
            <i className="fas fa-plus mr-1" style={{ marginRight: '5px' }}></i> Add Profile
          </button>
        </div>
      </div>
              ...
              {loading ? (
                ...
                profiles.map(p => (
                  <tr key={p.id} style={{ borderBottom: '1px solid #eee' }}>
                    <td style={{ padding: '15px 20px' }}><strong className="text-dark">{p.full_name}</strong></td>
                    <td style={{ padding: '15px 20px' }}>{p.email || '—'}</td>
                    <td style={{ padding: '15px 20px' }}>{p.phone || '—'}</td>
                    <td style={{ padding: '15px 20px' }}>{p.username || '—'}</td>
                    ...
                    <td style={{ padding: '15px 20px', textAlign: 'center' }}>
                      <button 
                        onClick={() => navigate(`/modules/organization-administration/employee-profile/${p.id}`)}
                ...
                <tr>
                  <td colSpan="6">
                    <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>
                       <i className="fas fa-address-card fa-3x mb-3 text-light" style={{ fontSize: '3rem', color: '#cbd5e1', marginBottom: '15px', display: 'block' }}></i>
                       <p className="mb-0">{query ? 'No matches.' : 'No profiles yet.'}</p>
                    </div>
                  </td>
                </tr>
              )}
