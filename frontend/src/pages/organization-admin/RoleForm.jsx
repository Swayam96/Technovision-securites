...
export default function RoleForm() {
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        const [coRes, depRes] = await Promise.all([
    ...
    if (isEdit) {
      axios.get(`/api/roles/${id}`, { withCredentials: true })
        .then(res => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await axios.put(`/api/roles/${id}`, formData, { withCredentials: true });
      } else {
        await axios.post('/api/roles', formData, { withCredentials: true });
      }
      toast.success(isEdit ? 'Updated successfully!' : 'Created successfully!');
      navigate('/modules/organization-administration/roles');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      <div className="content-header" style={{ marginBottom: '20px' }}>
        <div className="text-muted small mb-1" style={{ fontSize: '0.85rem' }}>Organisation & Administration &gt; Role</div>
      ...
      <form onSubmit={handleSubmit}>
        <div style={cardStyle}>
          <div style={cardHeaderStyle}>Role Details</div>
          <div style={cardBodyStyle}>
            <div style={gridStyle}>
              <div>
                <label style={labelStyle}>Role Name *</label>
                <input type="text" name="role_name" value={formData.role_name || ''} onChange={handleChange} required style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Role Code *</label>
                <input type="text" name="role_code" value={formData.role_code || ''} onChange={handleChange} required style={inputStyle} />
              </div>
              <div>
        ...
        <div style={{ textAlign: 'right', marginTop: '20px' }}>
          <button type="button" onClick={() => navigate('/modules/organization-administration/roles')} style={{ background: '#f8fafc', color: '#475569', border: '1px solid #cbd5e1', padding: '10px 20px', borderRadius: '6px', cursor: 'pointer', marginRight: '10px', fontWeight: 500 }}>
            Cancel
          </button>
