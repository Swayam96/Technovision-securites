...
export default function DesignationForm() {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      axios.get(`/api/designations/${id}`, { withCredentials: true })
        .then(res => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await axios.put(`/api/designations/${id}`, formData, { withCredentials: true });
      } else {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      <div className="content-header" style={{ marginBottom: '20px' }}>
        <div className="text-muted small mb-1" style={{ fontSize: '0.85rem' }}>Organisation & Administration &gt; Designation</div>
      ...
      <form onSubmit={handleSubmit}>
        <div style={cardStyle}>
          <div style={cardHeaderStyle}>Designation Details</div>
          <div style={cardBodyStyle}>
            <div style={gridStyle}>
              <div>
                <label style={labelStyle}>Designation Name *</label>
                <input type="text" name="designation_name" value={formData.designation_name || ''} onChange={handleChange} required style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Code *</label>
                <input type="text" name="designation_code" value={formData.designation_code || ''} onChange={handleChange} required style={inputStyle} />
              </div>
              <div>
        ...
        <div style={{ textAlign: 'right', marginTop: '20px' }}>
          <button type="button" onClick={() => navigate('/modules/organization-administration/designations')} style={{ background: '#f8fafc', color: '#475569', border: '1px solid #cbd5e1', padding: '10px 20px', borderRadius: '6px', cursor: 'pointer', marginRight: '10px', fontWeight: 500 }}>
            Cancel
          </button>
