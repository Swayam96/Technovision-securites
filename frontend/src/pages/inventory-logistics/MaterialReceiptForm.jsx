...
export default function MaterialReceiptForm() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`/api/inventory/receipts/${id}`, { withCredentials: true });
      const data = res.data;
      if (data.date) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id && id !== 'new') {
        await axios.put(`/api/inventory/receipts/${id}`, formData, { withCredentials: true });
      } else {
        await axios.post(`/api/inventory/receipts`, formData, { withCredentials: true });
      }
      navigate('/modules/inventory-logistics/receipts');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 text-gray-800 font-weight-bold">{id === 'new' ? 'Create' : 'Edit'} Material Receipt</h4>
      </div>
        ...
        <div className="card-body">
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-4">
              ...
              <button type="button" onClick={() => navigate('/modules/inventory-logistics/receipts')} className="btn btn-light px-4 border">Cancel</button>
            </div>
          </form>
        </div>
      </div>
