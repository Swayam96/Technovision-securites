...
export default function AMCForm() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`/api/service/amc/${id}`, { withCredentials: true });
      const data = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id && id !== 'new') {
        await axios.put(`/api/service/amc/${id}`, formData, { withCredentials: true });
      } else {
        await axios.post(`/api/service/amc`, formData, { withCredentials: true });
      }
      navigate('/modules/service-amc/amc');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="mb-0 text-gray-800 font-weight-bold">{id === 'new' ? 'Create' : 'Edit'} AMC</h4>
      </div>
        ...
        <div className="card-body">
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-4">
              ...
              <button type="button" onClick={() => navigate('/modules/service-amc/amc')} className="btn btn-light px-4 border">Cancel</button>
            </div>
          </form>
        </div>
      </div>
