...
export default function ServiceCallManagement() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get('/api/service/calls', { withCredentials: true });
      setData(res.data);
      setLoading(false);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h4 className="mb-0 text-gray-800 font-weight-bold">Service Call</h4>
        </div>
        <div>
          <Link to="/modules/service-amc/calls/new" className="btn btn-primary btn-sm px-3 shadow-sm rounded-pill">
                ...
                <tbody>
                  {data.length === 0 ? (
                    ...
                    data.map(item => (
                      <tr key={item.id}>
                        <td>{item.id}</td>
                        <td className="font-weight-bold">{item.call_no || item.id}</td>
                        <td>
                          <span className="badge bg-success rounded-pill px-3 py-2">{item.status || 'Active'}</span>
                        </td>
                        <td>
                          <Link to={`/modules/service-amc/calls/${item.id}`} className="btn btn-sm btn-link text-primary p-0 me-2"><i className="fas fa-edit"></i> Edit</Link>
