...
const dir = path.join(__dirname, '../frontend/src/pages/service-amc');
...
export default function ${componentName}Management() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get('/api/service/${apiPath}', { withCredentials: true });
      setData(res.data);
      setLoading(false);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <Link to="/modules/service-amc/${apiPath}/new" className="btn btn-primary shadow-sm">
                  ...
                  {data.length === 0 ? (
                    ...
                    data.map(item => (
                      <tr key={item.id}>
                        ...
                        <td>
                          <Link to={\`/modules/service-amc/${apiPath}/\${item.id}\`} className="btn btn-sm btn-link text-primary p-0 me-2"><i className="fas fa-edit"></i> Edit</Link>
...
export default function ${componentName}Form() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(\`/api/service/${apiPath}/\${id}\`, { withCredentials: true });
      const data = res.data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id && id !== 'new') {
        await axios.put(\`/api/service/${apiPath}/\${id}\`, formData, { withCredentials: true });
      } else {
        await axios.post(\`/api/service/${apiPath}\`, formData, { withCredentials: true });
      }
      navigate('/modules/service-amc/${apiPath}');
    } catch (err) {
      console.error(err);
        ...
        <div className="card-body">
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-4">
              ...
              <button type="button" onClick={() => navigate('/modules/service-amc/${apiPath}')} className="btn btn-light px-4 border">Cancel</button>
            </div>
          </form>
        </div>
      </div>
...
const configs = [
  { comp: 'ServiceRequest', path: 'requests', title: 'Service Request', idField: 'request_no' },
  { comp: 'ServiceCall', path: 'calls', title: 'Service Call', idField: 'call_no' },
  { comp: 'TechnicianAssignment', path: 'assignments', title: 'Technician Assignment', idField: 'assignment_no' },
  { comp: 'PreventiveMaintenance', path: 'maintenance', title: 'Preventive Maintenance', idField: 'pm_no' },
  { comp: 'AMC', path: 'amc', title: 'AMC', idField: 'amc_no' },
  { comp: 'Breakdown', path: 'breakdown', title: 'Breakdown / Complaint', idField: 'breakdown_no' },
  { comp: 'ServiceClosure', path: 'closures', title: 'Service Closure', idField: 'closure_no' },
];
...
console.log('Service & AMC components generated successfully.');
