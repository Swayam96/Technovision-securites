...
router.get('/dashboard', async (req, res) => {
    try {
        const result = {
            totalRevenue: "2.48 Cr",
            totalOrders: 186,
            projectsInProgress: 24,
            serviceContracts: 128,
            totalPurchaseValue: "1.32 Cr",
            inventoryValue: "78.5 L",
            outstandingReceivables: "62.4 L",
            netProfit: "28.6 L"
        };
        res.json(result);
    } catch (e) {
...
createCrudRoutes(router, '/reports', 'mis_reports');
createCrudRoutes(router, '/budget-planning', 'mis_budgets');
createCrudRoutes(router, '/kpis-scorecards', 'mis_kpi');
createCrudRoutes(router, '/management-review', 'mis_reviews');
